//! Behavioural conformance suite for [`Filesystem`] implementations.
//!
//! Every case is a plain async function over `&dyn Filesystem`; the macros
//! below instantiate each one as a `#[tokio::test]` per backend fixture. A
//! backend that cannot honour part of the contract fails here, with a name
//! that says what it got wrong, rather than under a guest with a `git`
//! stack trace.
//!
//! Out-of-tree backends run the same suite: depend on this crate with the
//! `conformance` feature in `dev-dependencies` and invoke the macros from a
//! test target. The tiers grade what a backend can promise:
//!
//! - [`conformance_core_cases!`]: the contract every backend must meet -
//!   files, directories, rename (including clobber and cycle refusal), and
//!   sandbox confinement. Object-store-shaped backends can meet this tier.
//! - [`conformance_portable_cases!`]: `core` plus hard links and explicit
//!   timestamps. Every backend with real inode semantics belongs here.
//! - [`conformance_posix_cases!`]: symbolic links, unlink-while-open, and
//!   the rename edge cases. The memory backend provides these everywhere;
//!   the host backend provides them on Unix.
//!
...
//! layer and are tested there; a [`File`](crate::File) handed out by a
//! backend is allowed to be as permissive as the memory backend's.

/// The conformance cases, one public async function per behaviour.
///
/// Case names are their documentation: each states the behaviour it asserts.
#[allow(missing_docs)]
pub mod cases {
    use std::sync::Arc;
    ...
    pub async fn open_err(fs: &dyn Filesystem, path: &str, open_flags: OF, flags: DF) -> ErrorCode {
        match open_with(fs, path, open_flags, flags, true).await {
    ...
    pub async fn create(fs: &dyn Filesystem, path: &str, contents: &[u8]) -> Arc<dyn File> {
    ...
    pub async fn mkdir(fs: &dyn Filesystem, path: &str) {
        fs.create_dir(p(path))
            .await
            .unwrap_or_else(|err| panic!("mkdir {path}: {err}"));
    }

    pub async fn read_all(file: &dyn File) -> Vec<u8> {
        let mut out = Vec::new();
    ...
    pub async fn create_write_read(fs: &dyn Filesystem) {
    ...
    pub async fn open_missing_fails(fs: &dyn Filesystem) {
        assert_eq!(
    ...
    pub async fn exclusive_create(fs: &dyn Filesystem) {
    ...
    pub async fn truncate_on_open(fs: &dyn Filesystem) {
    ...
    pub async fn set_size_truncates_and_extends(fs: &dyn Filesystem) {
    ...
    pub async fn write_past_end_zero_fills(fs: &dyn Filesystem) {
        let file = create(fs, "f", b"").await;
        file.write_at(b"xy", 4).await.unwrap();
        assert_eq!(read_all(&*file).await, b"\0\0\0\0xy");
    }

    pub async fn append_lands_at_end(fs: &dyn Filesystem) {
    ...
    pub async fn unlink_while_open_still_readable(fs: &dyn Filesystem) {
    ...
    pub async fn rename_replaces_existing_file(fs: &dyn Filesystem) {
    ...
    pub async fn rename_into_subdirectory(fs: &dyn Filesystem) {
        mkdir(fs, "d").await;
        create(fs, "f", b"x").await;
    ...
    pub async fn rename_dir_into_itself_refused(fs: &dyn Filesystem) {
        mkdir(fs, "d").await;
        mkdir(fs, "d/e").await;
    ...
    pub async fn read_dir_omits_dot_entries(fs: &dyn Filesystem) {
        create(fs, "a", b"").await;
        mkdir(fs, "sub").await;
    ...
    pub async fn create_and_remove_dir(fs: &dyn Filesystem) {
        mkdir(fs, "d").await;
    ...
    pub async fn open_directory_semantics(fs: &dyn Filesystem) {
        mkdir(fs, "d").await;
        assert!(matches!(
    ...
    pub async fn symlink_stat_follow_vs_nofollow(fs: &dyn Filesystem) {
    ...
    pub async fn symlink_open_nofollow_is_loop(fs: &dyn Filesystem) {
    ...
    pub async fn symlink_readlink(fs: &dyn Filesystem) {
    ...
    pub async fn symlink_over_existing_refused(fs: &dyn Filesystem) {
        create(fs, "f", b"").await;
        assert_eq!(
    ...
    pub async fn unlink_symlink_leaves_target(fs: &dyn Filesystem) {
        create(fs, "target", b"t").await;
        fs.symlink("target", p("link")).await.unwrap();
    ...
    pub async fn symlink_cycle_is_loop(fs: &dyn Filesystem) {
    ...
    pub async fn dotdot_at_root_not_permitted(fs: &dyn Filesystem) {
        assert_eq!(
    ...
    pub async fn absolute_symlink_target_refused(fs: &dyn Filesystem) {
    ...
    pub async fn escaping_symlink_refused(fs: &dyn Filesystem) {
    ...
    pub async fn dotdot_through_symlink_lands_in_target_parent(fs: &dyn Filesystem) {
        mkdir(fs, "dir1").await;
        mkdir(fs, "dir2").await;
    ...
    pub async fn create_through_dangling_symlink(fs: &dyn Filesystem) {
        mkdir(fs, "d").await;
    ...
    pub async fn metadata_hash_tracks_changes(fs: &dyn Filesystem) {
    ...
    pub async fn set_times_explicit(fs: &dyn Filesystem) {
        create(fs, "f", b"").await;
...
/// Instantiates one `#[tokio::test]` per named case against `$fixture`.
///
/// `$fixture` is a function returning `(filesystem, guard)`; the guard is
/// dropped after the case runs and exists for fixtures that need cleanup
/// (temporary directories, temporary databases).
#[macro_export]
    ...
    ($fixture:path => $($case:ident),* $(,)?) => {
        $(
            ...
            async fn $case() {
                ...
                $crate::conformance::cases::$case(&fs).await;
            }
        )*
    };
}

/// The tier every backend must pass: files, directories, rename semantics,
/// and sandbox confinement. Requires neither link support (hard or symbolic)
/// nor settable timestamps, so object-store-shaped backends qualify.
#[macro_export]
macro_rules! conformance_core_cases {
    ($fixture:path) => {
...
/// The `core` tier plus hard links and explicit timestamps: everything that
/// holds on any platform for a backend with real inode semantics.
#[macro_export]
macro_rules! conformance_portable_cases {
    ($fixture:path) => {
...
/// files, and rename between hard links. The memory backend provides them
/// everywhere.
#[macro_export]
macro_rules! conformance_posix_cases {
    ($fixture:path) => {
        $crate::conformance_suite!($fixture =>
